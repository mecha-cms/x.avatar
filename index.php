...
function avatar($avatar, array $lot = []) {
    if ($avatar) {
        ...
        if (isset($state->x->image) && $lot) {
            if (0 === \strpos($avatar, '/') && 0 !== \strpos($avatar, '//')) {
                $width = \ceil($lot[0]);
                ...
                $blob->crop($width, $height)->blob($store, $quality); // Generate avatar cache
                $avatar = \To::URL($store); // Return the avatar cache URL
                // Convert direct avatar URL from folder `.\lot\image` to its proxy avatar URL
                if ($avatar && 0 === \strpos($avatar, $url . '/lot/image/')) {
